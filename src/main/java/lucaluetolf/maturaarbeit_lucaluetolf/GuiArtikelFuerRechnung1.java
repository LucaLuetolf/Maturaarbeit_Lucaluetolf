...
public class GuiArtikelFuerRechnung1 extends GuiTaskleiste implements Initializable {
    Statement statement;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try {
            ...
            for (int i = 0; i < laenge; i++) {
                ...
                ResultSet resultsetAnzahl = statement.executeQuery("SELECT COUNT(*) FROM bestellung WHERE artikel_Id = " + id + " AND " + rechnungsnummer + " = bestellungId");
                resultsetAnzahl.next();
                if (resultsetAnzahl.getInt(1) != 0){
                    resultsetAnzahl.close();
                    ResultSet resultsetAnzahl1 = statement.executeQuery("SELECT * FROM bestellung WHERE artikel_Id = " + id + " AND " + rechnungsnummer + " = bestellungId");
                ...
                ResultSet resultSetExistiertKunde = statement.executeQuery("SELECT * FROM bearbeiter WHERE bestellung_id = " + rechnungsnummer);
        ...
        try {
            ...
            if (anzahl == 0) {
                ...
                if (resultSetUeberpruefen.getInt("summe") == 0) {
                    statement.execute("INSERT INTO bestellung (bestellungId, artikel_id, anzahl, rabatt) VALUES (" + rechnungsnummer + "," + artikelnummer + "," + anzahl + "," + rabatt + ")");
                } else {
            ...
            ResultSet resultSetSumme = statement.executeQuery("SELECT SUM(anzahl * preis) AS summe FROM bestellung, artikel WHERE " + rechnungsnummer +" = bestellungId AND artikelId = artikel_id");
    ...
    protected void buttonRechnungErstellen(ActionEvent event) {
        ...
        try {
            ResultSet resultSetLagerbestand = statement.executeQuery("SELECT * FROM bestellung WHERE " + rechnungsnummer + " = bestellungId");
            ...
            int rechnungsnummer1 = 0;
            ResultSet resultSetRechnungsnummer = statement.executeQuery("SELECT * FROM unternehmen");
            if(resultSetRechnungsnummer.next()){
                rechnungsnummer1 = resultSetRechnungsnummer.getInt("rechnungsnummer");
            }
            if (rechnungsnummer1 == rechnungsnummer){
